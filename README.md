...
`cases/` 是 benchmark case 的声明层，负责定义每个可运行实例的元数据、问题类型、规模信息、建模方式和结果解码逻辑。这里不存放 dashboard 结果，只存放 `benchmarks.run`、suite runner 和 dashboard 能共同理解的 case 定义。666666
