...
NUG12 = QapCase(
    ...
    family='sequence_quadratic_assignment',
    tier='smoke',
    compare_key=f"{SOURCE_KEY}/{PROBLEM_TYPE}/{INSTANCE_TYPE}/nug12",
    series_key=f"{SOURCE_KEY}/{PROBLEM_TYPE}/{INSTANCE_TYPE}/nug12/{MODEL_STYLE}",
    size={'facilities': 12, 'locations': 12},
 ...
 'solution_url': 'https://qaplib.mgi.polymtl.ca/soln.d/nug12.sln'},
    reference={'objective': 578,
 'source_label': 'Nug12',
 'source_url': 'https://qaplib.mgi.polymtl.ca/',
 'status': 'optimal',
 'value_kind': 'optimal'},
    problem_description='QAPLIB quadratic assignment instance nug12: assign 12 facilities to 12 locations. The cost is the sum of flow between facility pairs multiplied by distance between assigned locations. This is a permutation blackbox benchmark with a published optimum.',
 ...
 'decision_variables': ['one sequence_var assignment of size 12; assignment[i] is the location '
...
NUG20 = QapCase(
    ...
    family='sequence_quadratic_assignment',
    tier='calibration',
    compare_key=f"{SOURCE_KEY}/{PROBLEM_TYPE}/{INSTANCE_TYPE}/nug20",
    series_key=f"{SOURCE_KEY}/{PROBLEM_TYPE}/{INSTANCE_TYPE}/nug20/{MODEL_STYLE}",
 ...
 'solution_url': 'https://qaplib.mgi.polymtl.ca/soln.d/nug20.sln'},
    reference={'objective': 2570,
 'source_label': 'Nug20',
 'source_url': 'https://qaplib.mgi.polymtl.ca/',
 'status': 'optimal',
 'value_kind': 'optimal'},
    problem_description='QAPLIB quadratic assignment instance nug20: assign 20 facilities to 20 locations. The cost is the sum of flow between facility pairs multiplied by distance between assigned locations. This is a permutation blackbox benchmark with a published optimum.',
    case_module=CASE_MODULE,
    modeling_notes={
        "model_style": MODEL_STYLE,
        "objective_sense": 'minimize',
    ...
    extra={
        ...
        "optagent_modeling": {'constraints': ['sequence_var enforces a one-to-one facility-location assignment'],
 'data_mapping': 'Read QAPLIB flow and distance matrices.',
 'decision_variables': ['one sequence_var assignment of size 20; assignment[i] is the location '
                        'chosen for facility i'],
 'external_callback': 'qap_cost(ctx) computes sum(flow[i][j] * '
                      'distance[assignment[i]][assignment[j]]) over all facility pairs.',
 'objective': "builder.minimize(builder.external_call(qap_cost, name='assignment_cost'), "
