...
```text
benchmarks/cases/qaplib/quadratic_assignment/raw/qapdata/
benchmarks/cases/qaplib/quadratic_assignment/raw/qapsoln/
```

筛选后正式注册 case 的 `.dat` 和 `.sln` 文件会复制到 `quadratic_assignment/raw/` 根层，保证 `--no-download` 时可直接运行。
