...
DMU01 = JobShopCase(
    ...
    family='interval_job_shop',
    tier='full',
    compare_key=f"{SOURCE_KEY}/{PROBLEM_TYPE}/{INSTANCE_TYPE}/dmu01",
    series_key=f"{SOURCE_KEY}/{PROBLEM_TYPE}/{INSTANCE_TYPE}/dmu01/{MODEL_STYLE}",
 ...
 'upper_bound': 2563,
 'value_kind': 'optimal'},
    problem_description='JSPLIB job-shop instance dmu01: schedule 20 jobs across 15 machines. Each job has a fixed machine route and fixed operation durations; each machine can process at most one operation at a time. The benchmark objective is minimum makespan.',
 ...
 'decision_variables': ['300 interval_var operation[j,k] with fixed duration and bounded start',
                        '15 sequence_var machine_order[m], each ordering operations assigned to '
                        'one machine'],
...
def solve_dmu01(**kwargs: Any) -> list[dict[str, Any]]:
    return solve_case("dmu01", **kwargs)
...
def _case_by_name(instance: str) -> JobShopCase:
    ...
    raise KeyError(f"unsupported DMU JSPLIB instance: {instance}; supported: {supported}")
