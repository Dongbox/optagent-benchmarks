...
FT06 = JobShopCase(
    ...
    family='interval_job_shop',
    tier='smoke',
    compare_key=f"{SOURCE_KEY}/{PROBLEM_TYPE}/{INSTANCE_TYPE}/ft06",
    series_key=f"{SOURCE_KEY}/{PROBLEM_TYPE}/{INSTANCE_TYPE}/ft06/{MODEL_STYLE}",
 ...
 'upper_bound': 55,
 'value_kind': 'optimal'},
    problem_description='JSPLIB job-shop instance ft06: schedule 6 jobs across 6 machines. Each job has a fixed machine route and fixed operation durations; each machine can process at most one operation at a time. The benchmark objective is minimum makespan.',
 ...
 'decision_variables': ['36 interval_var operation[j,k] with fixed duration and bounded start',
                        '6 sequence_var machine_order[m], each ordering operations assigned to one '
                        'machine'],
...
FT10 = JobShopCase(
    ...
    family='interval_job_shop',
    tier='smoke',
    compare_key=f"{SOURCE_KEY}/{PROBLEM_TYPE}/{INSTANCE_TYPE}/ft10",
    series_key=f"{SOURCE_KEY}/{PROBLEM_TYPE}/{INSTANCE_TYPE}/ft10/{MODEL_STYLE}",
 ...
 'upper_bound': 930,
 'value_kind': 'optimal'},
    problem_description='JSPLIB job-shop instance ft10: schedule 10 jobs across 10 machines. Each job has a fixed machine route and fixed operation durations; each machine can process at most one operation at a time. The benchmark objective is minimum makespan.',
    case_module=CASE_MODULE,
    modeling_notes={
        "model_style": MODEL_STYLE,
        "objective_sense": 'minimize',
 ...
 'data_mapping': 'Read ScheduleOpt JSPLIB JSON rows as operations with job, operation index, '
                 'machine, and duration.',
 'decision_variables': ['100 interval_var operation[j,k] with fixed duration and bounded start',
...
def solve_ft06(**kwargs: Any) -> list[dict[str, Any]]:
    return solve_case("ft06", **kwargs)


def solve_ft10(**kwargs: Any) -> list[dict[str, Any]]:
    return solve_case("ft10", **kwargs)
...
def _case_by_name(instance: str) -> JobShopCase:
    ...
    raise KeyError(f"unsupported FT JSPLIB instance: {instance}; supported: {supported}")
