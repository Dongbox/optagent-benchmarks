...
LA16 = JobShopCase(
    ...
    family='interval_job_shop',
    tier='calibration',
    compare_key=f"{SOURCE_KEY}/{PROBLEM_TYPE}/{INSTANCE_TYPE}/la16",
    series_key=f"{SOURCE_KEY}/{PROBLEM_TYPE}/{INSTANCE_TYPE}/la16/{MODEL_STYLE}",
 ...
 'upper_bound': 945,
 'value_kind': 'optimal'},
    problem_description='JSPLIB job-shop instance la16: schedule 10 jobs across 10 machines. Each job has a fixed machine route and fixed operation durations; each machine can process at most one operation at a time. The benchmark objective is minimum makespan.',
...
LA21 = JobShopCase(
    ...
    family='interval_job_shop',
    tier='calibration',
    compare_key=f"{SOURCE_KEY}/{PROBLEM_TYPE}/{INSTANCE_TYPE}/la21",
    series_key=f"{SOURCE_KEY}/{PROBLEM_TYPE}/{INSTANCE_TYPE}/la21/{MODEL_STYLE}",
    size={'jobs': 15, 'machines': 10, 'operations': 150},
    data={'documentation_url': 'https://scheduleopt.github.io/benchmarks/jsplib/',
 'instance_url': 'https://raw.githubusercontent.com/ScheduleOpt/benchmarks/main/jobshop/instances/json/la21.json',
 'solution_url': 'https://raw.githubusercontent.com/ScheduleOpt/benchmarks/main/jobshop/solutions/bks.json'},
    reference={'lower_bound': 1046,
 'objective': 1046,
 'reported_machine': 'i7-1185G7 @ 3.00GHz',
 'reported_solver': 'OptalCP',
 'reported_time_seconds': 1,
 ...
 'upper_bound': 1046,
 'value_kind': 'optimal'},
    problem_description='JSPLIB job-shop instance la21: schedule 15 jobs across 10 machines. Each job has a fixed machine route and fixed operation durations; each machine can process at most one operation at a time. The benchmark objective is minimum makespan.',
    case_module=CASE_MODULE,
    modeling_notes={
        "model_style": MODEL_STYLE,
        "objective_sense": 'minimize',
 ...
 'data_mapping': 'Read ScheduleOpt JSPLIB JSON rows as operations with job, operation index, '
                 'machine, and duration.',
 'decision_variables': ['150 interval_var operation[j,k] with fixed duration and bounded start',
                        '10 sequence_var machine_order[m], each ordering operations assigned to '
                        'one machine'],
 'objective': 'builder.minimize(builder.max(*(builder.interval_end(last_operation[j]) for each '
              "job)), name='makespan')",
