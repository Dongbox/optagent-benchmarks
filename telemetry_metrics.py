...
def derive_five_dimensional_metrics(
    ...
    *,
    ...
    return {
        ...
        "robustness": calculate_robustness(dataset),
        "anytime": calculate_anytime(dataset, references=normalized_refs, targets=targets or {}),
        "statistical_validity": calculate_statistical_validity(dataset),
    }
...
def calculate_effectiveness(
    ...
    *,
    ...
    for strategy, rows in dataset.rows_by_strategy().items():
        ...
        gaps = [
            _relative_gap(row.objective, references[row.instance_id], row.objective_sense)
            for row in feasible
            if row.objective is not None and row.instance_id in references
        ]
        by_strategy[strategy] = {
            ...
            "best_objective": _numeric_metric(min(objectives) if objectives else None, rows, "objective"),
...
def calculate_robustness(dataset: MetricDataset) -> dict[str, Any]:
    by_strategy: dict[str, dict[str, Any]] = {}
    for strategy, rows in dataset.rows_by_strategy().items():
    ...
    for strategy, rows in dataset.rows_by_strategy().items():
        ...
        for row in rows:
            ...
            if reference is not None:
                integral = primal_integral(curve, reference, row.time_budget_s or row.wall_time_s, row.objective_sense)
        ...
        by_strategy[strategy] = {
            ...
            if time_to_targets
                ...
                None,
                availability=INSUFFICIENT_DATA,
                reason="no_target_hit_or_missing_target",
                source="benchmarks",
...
def calculate_statistical_validity(dataset: MetricDataset) -> dict[str, Any]:
    ...
    for left_index, left in enumerate(strategies):
        for right in strategies[left_index + 1 :]:
            left_values, right_values = matched_objectives(dataset, left, right)
            pairs.append(
                {
...
def friedman_test(dataset: MetricDataset) -> dict[str, Any]:
    ...
    for instance in matched_instances:
        values = [(strategy, _mean(per_instance[instance][strategy])) for strategy in strategies]
