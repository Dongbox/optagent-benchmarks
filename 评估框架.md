...
本文档定义 OptAgent 搜索策略的**综合 benchmark 评估体系**，提供科学评估、算子级分析和多维性能特征刻画。

## 与策略评分的关系

- **策略评分**（`策略评分.md`）= CI 回归指标（快速、单一标量、决定 PASS/FAIL）
- **评估框架**（本文档）= 研究评估系统（综合、多视角、解释性证据）

**评估框架用于解释和分析，不控制 CI PASS/FAIL。** CI 决策由策略评分负责。

评估实现见 [`scoring.py`](scoring.py)。
